...
class Kohonen:
    ...
    def train(self, data):
        ...
        for t in range(self.END):
            ...
            # Calculate the distances between the units and the input.
            dists_x = np.linalg.norm(units - x, axis = 1)
            # Find the best match unit.
            min_unit_num = dists_x.argmin()

            # Calculate the distances between the best match unit and units on the map.
            dists_p = np.linalg.norm(self.pos - self.pos[min_unit_num], axis=1)
            # Move the reference vectors of the units to the input.
